...
/* V5 -- Inherited from V4 -- Improving by decreasing the disparity between servo readings, by reading the servo values as close as possible to each other [w.r.t time at which they are read] */
...
{ 
  ...
  enum message_type{Request,Command};
  ...
  if(Serial.available())
  {
    ...
    if(mType_available && to_available)
    {
      int servo_add = 0;
      servo_add = atoi(to);
