...
// determine if we use ROS - comment not to use it
//#include "ad_core/definitions/maggieoptions.h"
#if AD_USE_ROS
#include <ros/ros.h>
...
inline void read_calibration_file(const std::string & calibration_filename,
  ...
  // printf("intrinsics:%ix%i\n", intrinsics.cols, intrinsics.rows);
...
*/
inline cv::Vec3f pixel2world_line_vector(const cv::Mat & intrinsics_inv,
...
*/
inline cv::Vec3f pixel2world_line_vector(const cv::Mat & intrinsics_inv, const cv::Point & pixel) {
...
*/
inline cv::Point3f pixel2world_known_distance(const cv::Mat & intrinsics_inv,
...
*/
inline cv::Point world2pixel(const cv::Mat & intrinsics,
...
/*!
  ...
  \see world2pixel()
*/
inline cv::Point world2pixel(const cv::Mat & intrinsics, const cv::Point3f & world) {
...
inline void mouse_cb(int event, int x, int y, int flags, void* userdata) {
  cv::Point* test_pixel = (cv::Point*) userdata;
  test_pixel->x = x;
  test_pixel->y = y;
}

inline void rectify_image_skeleton(cv::VideoCapture & capture,
  ...
  // set callback to be able to use the mouse in the window
  // CV_EXPORTS void setMouseCallback(const string& winname, MouseCallback onMouse, void* userdata = 0);
  cv::setMouseCallback("frame_remapped", mouse_cb, &test_pixel);

  bool init_test_pixel = true;
  double fixed_height = 1.29;
  while (true) {
    ...
    double arrow_length = .3;
    cv::line(frame_remapped,
             world2pixel(intrinsics, 0, 0, fixed_height),
             world2pixel(intrinsics, arrow_length, 0, fixed_height),
             CV_RGB(255, 0, 0), 2);
    cv::line(frame_remapped,
             world2pixel(intrinsics, 0, 0, fixed_height),
             world2pixel(intrinsics, 0, arrow_length, fixed_height),
             CV_RGB(0, 255, 0), 2);
    cv::line(frame_remapped,
             world2pixel(intrinsics, 0, 0, fixed_height),
             world2pixel(intrinsics, arrow_length, arrow_length, fixed_height + arrow_length),
             CV_RGB(0, 0, 255), 2);
    ...
    cv::Point3f p3D = pixel2world_known_distance(intrinsics_inv, test_pixel, fixed_height);
    std::ostringstream txt; txt << "p3D:" << p3D << " (p:" << test_pixel << ")";
    cv::circle(frame_remapped, test_pixel, 2, CV_RGB(255, 0, 0), -1);
...
/*!
 ...
 \param calibration_filename
    the file containing the intrinsic and extrinsic parameters of the camera
*/
inline void rectify_image_ros(cv::VideoCapture & capture,
                       const std::string & calibration_filename) {
#if AD_USE_ROS
  int argc = 0;
